...
public class CosmosDbAdapterTests
{
	...
	public async Task Adapter_CanBeUsedWithCosmosDbInterface()
	{
		...
		await adapter.AddContainerAsync(_containerName);

		// Act - Add an item
		var testItem = new { id = "test-id", Name = "Test Item", Value = 42 };
		await adapter.AddItemAsync(_containerName, testItem);

		// Act - Query the item
		var results = await adapter.QueryAsync(_containerName, "SELECT * FROM c WHERE c.id = 'test-id'");
	...
	public async Task Adapter_SupportsPagination()
	{
		...
		for (int i = 1; i <= 20; i++)
		{
			await adapter.AddItemAsync(_containerName, new
			{
				id = i.ToString(),
				Name = $"Item {i}",
				Value = i
		...
		var (page1Results, page1Token) = await adapter.QueryWithPaginationAsync(_containerName, "SELECT * FROM c", 5);
		...
		// Act - Get second page
		var (page2Results, page2Token) = await adapter.QueryWithPaginationAsync(_containerName, "SELECT * FROM c", 5, page1Token);
	...
	public async Task Adapter_HandlesAdvancedQueries()
	{
		...
		var containsResults = await adapter.QueryAsync(_containerName, "SELECT * FROM c WHERE CONTAINS(c.Name, 'Product')");

		// Assert
		Assert.Equal(3, containsResults.Count());

		// Act - Query with comparison and projection
		var projectionResults = await adapter.QueryAsync(_containerName, "SELECT c.Name, c.Price FROM c WHERE c.Price < 100");
